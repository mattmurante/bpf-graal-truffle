...
@NodeInfo(language = "BPF", description = "STB operation")
public abstract class STB extends MemExpressionNode {
	
	public STB(byte opcode, byte srcReg, byte destReg, short offset, int imm) {
	...
	public boolean operation(@CachedLanguage BPFLanguage language) {
		...
		memory.setByte((regs[destReg] + offset), (byte) imm);
		return true;
	}
