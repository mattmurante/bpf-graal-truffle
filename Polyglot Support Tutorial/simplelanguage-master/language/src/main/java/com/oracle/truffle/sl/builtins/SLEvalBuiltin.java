...
public abstract class SLEvalBuiltin extends SLBuiltinNode {
    ...
    @TruffleBoundary
    @Specialization
    public Object evaluate(String id, String code, @CachedContext(SLLanguage.class) SLContext context) {
    ...
    protected CallTarget parse(String id, String code, SLContext context) {
		...
		if (id == "bpf") {
		    ...
		    try {
		    	...
		    	System.err.println("BPF program not found");
		    }
		    source = Source.newBuilder(id, ByteSequence.create(bytes), "(eval)").build();
		}
		else {
		    source = Source.newBuilder(id, code, "(eval)").build();
		}
